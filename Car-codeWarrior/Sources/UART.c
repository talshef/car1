...
void InitUARTs(){
	...
	UART0_C2 = UARTLP_C2_TE_MASK | UART_C2_RIE_MASK; // Enable Transmitter,  Receive interrupt
	set_irq_priority(INT_UART0-16,4);
...
void UART0_IRQHandler(){
	...
	if((UART0_S1 & UART_S1_TDRE_MASK)&(UART0_C2  & UART_C2_TIE_MASK)){ // TX buffer is empty and ready for writing
		 ...
		 UART_D_REG(UART0_BASE_PTR)  =Dequeue();
		if(isEmpty()) UART0_C2 &=   ~UART_C2_TIE_MASK;		
